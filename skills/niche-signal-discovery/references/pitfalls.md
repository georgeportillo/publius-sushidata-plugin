...
5. **Running analysis immediately after enrichment** — WebFetch and Apify calls complete synchronously; however, large CSV writes can buffer. Run the file completeness check in Step 3 before executing `analyze_signals.py`. A `won_with_jobs: 0` result when you expect data is the symptom; re-running the analysis (without re-enriching) fixes it.
6. **Duplicate domains in input** — CRM exports often have the same company in both won and lost (multiple deals). Sushidata enrichment should treat each apex domain as one company-level entity; duplicate rows can split or undercount job data across won/lost groups. Always deduplicate in Step 1.
